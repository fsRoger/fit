...
import { INITIAL_TASKS, INITIAL_PANTRY, INITIAL_RECIPES } from "@/lib/data";
import type {
  Task,
  ShoppingItem,
  PantryItem,
...
export default function NutriMassApp() {
  ...
  const [shopping, setShopping] = useLocalState<ShoppingItem[]>(
    "nm_shopping",
    [],
  );
  const [pantry, setPantry] = useLocalState<PantryItem[]>(
    "nm_pantry",
    INITIAL_PANTRY,
  );
  const [log, setLog] = useLocalState<MealLog>("nm_log", {});

  const [recipes, setRecipes] = useLocalState<Recipe[]>(
    "nm_recipes",
    INITIAL_RECIPES,
  );
