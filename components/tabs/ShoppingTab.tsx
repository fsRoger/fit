...
import { ShoppingItem, UNITS } from '@/lib/data'
...
export default function ShoppingTab({ shopping, setShopping }: Props) {
  ...
  const add = () => {
    ...
    setShopping((p) => [
      ...p,
      { id: Date.now().toString(), name: name.trim(), qty: qty || '1', unit, checked: false },
    ])
    setName('')
    setQty('')
  }
  ...
  const toggle = (id: string) =>
    setShopping((p) => p.map((i) => (i.id === id ? { ...i, checked: !i.checked } : i)))

  const remove = (id: string) => setShopping((p) => p.filter((i) => i.id !== id))
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="space-y-2">
        {[...pending, ...checked].map((i) => (
          <div
            ...
            className={`flex items-center gap-3 p-4 rounded-xl border transition-all ${
              i.checked ? 'bg-gray-900/30 border-gray-800 opacity-60' : 'bg-gray-800/60 border-gray-700'
            }`}
          >
            <button
              onClick={() => toggle(i.id)}
              className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center flex-shrink-0 transition-all ${
                i.checked ? 'bg-amber-500 border-amber-500' : 'border-gray-600 hover:border-amber-500'
              }`}
            >
              {i.checked && <Check size={14} className="text-white" />}
            </button>
            <span className={`flex-1 text-sm ${i.checked ? 'line-through text-gray-500' : 'text-gray-200'}`}>
              {i.name}
            </span>
        ...
        {shopping.length === 0 && (
          <div className="text-center text-gray-500 py-12">Lista vazia. Adicione itens acima!</div>
        )}
      </div>
    </div>
  )
